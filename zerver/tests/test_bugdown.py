...
class BugdownTest(ZulipTestCase):
    ...
    def test_thumbor_external_link(self):
        ...
        src_pattern = re.compile(r'(src|data-original)=\"([\S]+)\"')
        with self.settings(THUMBOR_HOST=thumbor_host, CAMO_URI=''):
            ...
            self.assertEqual(len(image_links[0]), 2)
            self.assertEqual(len(image_links[1]), 2)
            image_links = [urllib.parse.unquote(l[1]) for l in image_links]
